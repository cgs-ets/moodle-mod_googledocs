...
$event = \mod_googledrive\event\course_module_instance_list_viewed::create($params);
...
if (! $googledrives = get_all_instances_in_course('googledrive', $course)) {
    notice(get_string('nogoogledrives', 'googledrive'), new moodle_url('/course/view.php', array('id' => $course->id)));
}
...
$currentsection = '';
foreach ($modinfo->instances['googledrive'] as $cm) {
    $row = array();
    if ($usesections) {
